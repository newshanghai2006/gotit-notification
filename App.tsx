...
function Login({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  const resetCaptcha = () => { setCaptcha(makeCaptcha()); setCaptchaInput(''); };
  ...
  const requestCode = async () => {
    ...
    setBusy(true);
    try { await api('/auth/request-code', undefined, { method: 'POST', body: { email } }); setSent(true); resetCaptcha(); Alert.alert(copy.sent, copy.sentMessage); }
  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.loginScroll} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets>
        ...
        <TextInput style={styles.input} placeholder={copy.emailPlaceholder} autoCapitalize="none" keyboardType="email-address" returnKeyType="next" value={email} onChangeText={setEmail} />
        <Text style={styles.label}>{copy.captcha}</Text>
...
function Inbox({ token, logout }: { token: string; logout: () => void }) {
  ...
  useEffect(() => { load(); registerPush(token).catch((error) => Alert.alert('通知注册失败', `${error instanceof Error ? error.message : String(error)}\n\n请检查 Firebase/FCM 配置和系统通知权限。`)); }, [token]);
  useEffect(() => {
